...
#-- if both logs exists
function both_ () {
echo -e " ${USER} :    " >> /var/www/lcms/modules/restarthttpd/${FILE}
cat /var/weblogs/${HIP}_${HN}/stat/${STAT} >> /var/www/lcms/modules/restarthttpd/${FILE}
LL="$(awk 'END{print}' /var/www/lcms/modules/restarthttpd/${FILE})"
...
cat >/var/www/lcms/modules/restarthttpd/${FILE}  <<-"EOF"
LOG:
START >> 
EOF
...
echo -e " ${USER} :    " >> /var/www/lcms/modules/restarthttpd/${FILE}
cat /var/weblogs/${HIP}_${HN}/stat/${STAT} >> /var/www/lcms/modules/restarthttpd/${FILE}
cat /var/www/lcms/modules/restarthttpd/${FILE} >> /var/www/lcms/modules/${STAT}
}

#-- check which log exists
CHECKF="$(cat /var/www/lcms/modules/restarthttpd/$FILE | wc -l)"
